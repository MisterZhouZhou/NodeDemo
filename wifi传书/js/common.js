...
var fileTypes = "txt|epub|ebk2|ebk3|pdf|umd|doc|docx|wps|xls|xlsx|et|ppt|pptx|dps";
...
//全局的配置文件
var config = {
	fileTypes : fileTypes, //允许上传的文件格式 "txt|epub"
	// url : "http://"+location.host+"?action=addBook",//"http://localhost/t/post.php",//
	url: "http://localhost:8888/file_upload",
...
function checkFile(file) {
    if(document.title.match('WiFi 字体')) {
        ...
        return checkBook(file)
    }
}

function checkBook(file) {
    if(file.size > config.fileLimitSize){
        return "文件不能超过500MB，大文件请参照“设置”-“用户指南”-“传书功能”同步到手机内查看";
    }
